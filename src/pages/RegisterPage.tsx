...
export function RegisterPage({ onToggle }: { onToggle: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signUpError) {
      ...
      if (signUpError.message?.includes('already registered') || signUpError.message?.includes('already exists')) {
        setError('Bu email adresi zaten kayıtlı. Lütfen giriş yapın.');
      } else {
        const errorMessage = signUpError.message || 'Kayıt başarısız. Lütfen tekrar deneyin.';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative">
      ...
      <div className="glass rounded-3xl shadow-2xl shadow-white/10 w-full max-w-md p-10 relative z-10">
        ...
        <p className="text-center text-white/80 mb-8 font-light">Yeni Hesap Oluştur</p>
        ...
        {success && (
          <div className="glass-dark border-green-400/50 text-green-200 px-4 py-3 rounded-xl mb-4">
            ...
            <p className="text-sm">Yöneticimiz size bir rol atadıktan sonra eğitim portalına tam erişim sağlayabileceksiniz. Şimdi giriş yapabilirsiniz.</p>
          </div>
        )}
